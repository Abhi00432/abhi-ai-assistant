...
# ----------------------------------------------------
# 2. Elite Animated Cyber-Neon Glass UI Engine
# ----------------------------------------------------
st.markdown("""
<style>
    ...
    * {
    ...
    /* Ambient Dynamic Aurora Mesh Animation */
    .stApp {
        background: radial-gradient(circle at 15% 15%, rgba(0, 240, 255, 0.07) 0%, transparent 40%),
                    radial-gradient(circle at 85% 85%, rgba(139, 92, 246, 0.08) 0%, transparent 45%),
                    linear-gradient(180deg, #030712 0%, #080d1e 50%, #02050e 100%);
    ...
    @keyframes neonBlink {
        0%, 100% { transform: scale(0.9); opacity: 0.7; box-shadow: 0 0 4px var(--neon-cyan); }
        50% { transform: scale(1.2); opacity: 1; box-shadow: 0 0 16px var(--neon-cyan), 0 0 25px var(--neon-purple); }
    }

    /* 3D Glass Chat Cards with Glow Hover */
    [data-testid="stChatMessage"] {
        background: rgba(14, 21, 44, 0.55) !important;
        ...
        animation: messageFloatIn 0.4s ease-out;
    }
    ...
    [data-testid="stChatMessage"]:hover {
        ...
        box-shadow: 0 12px 35px rgba(0, 240, 255, 0.1), 0 0 15px rgba(139, 92, 246, 0.08);
    ...
    .auth-card {
        ...
        box-shadow: 0 25px 50px rgba(0, 0, 0, 0.7), 0 0 30px rgba(0, 240, 255, 0.1);
    ...
    /* Glowing Cyber Gradient Buttons */
    .stButton>button {
        background: linear-gradient(135deg, rgba(0, 240, 255, 0.9) 0%, rgba(59, 130, 246, 0.9) 50%, rgba(139, 92, 246, 0.9) 100%) !important;
...
def create_new_session(email: str, title: str = "New Session") -> int:
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
...
# ----------------------------------------------------
# 6. Cyber Hologram Login Screen
# ----------------------------------------------------
if not st.session_state.authenticated_user:
    st.markdown("""
    <div class='auth-card'>
        <div style='text-align: center; margin-bottom: 25px;'>
            <div class='pulse-laser'></div>
            <span style='font-size: 0.8rem; letter-spacing: 2px; text-transform: uppercase; color: #00f0ff;'>Secure Workspace</span>
            <h2 style='margin: 10px 0 4px 0; font-weight: 800; font-size: 1.8rem; letter-spacing: -0.5px;'>AI PORTAL</h2>
...
# ----------------------------------------------------
# 8. Holographic Sidebar (Chats & Controls)
# ----------------------------------------------------
with st.sidebar:
...
# ----------------------------------------------------
# 10. High-Precision Neural Execution Engine
# ----------------------------------------------------
if user_query:
    ...
    with st.chat_message("assistant"):
        ...
        # ROUTE 1: Text-To-Image Synthesis
        ...
        # ROUTE 2: Vision OCR & Diagram Reasoning
        elif base64_img:
        ...
        # ROUTE 3: High-Precision STEM Reasoning (Qwen 3B)
        else:
            system_instruction = {
                "role": "system",
                "content": "You are an expert AI mathematician, computer scientist, and general knowledge engine. Provide mathematically rigorous, step-by-step, clean, and verified answers."
            }
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    st.error(f"Inference Server Alert: Status {response.status_code}")
            except Exception as ex:
